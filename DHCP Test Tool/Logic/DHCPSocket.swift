...
enum DHCPSocket {
    nonisolated static func sendDiscoverAndCollectResponses(
        ...
        let perAttemptTimeout = max(0.2, timeout / Double(resendAttempts))
        
        for _ in 0..<resendAttempts {
            if packets.count >= maxResponses || Date() >= overallDeadline {
                break
            }
            ...
            let attemptDeadline = min(overallDeadline, Date().addingTimeInterval(perAttemptTimeout))
            while packets.count < maxResponses && Date() < attemptDeadline {
                let remaining = attemptDeadline.timeIntervalSinceNow
                if remaining <= 0 {
                    break
                }
